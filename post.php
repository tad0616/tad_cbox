...
function tad_cbox_form($sn = '')
{
    ...
    //判斷是否需要登入才能留言
    if (empty($xoopsUser) and '1' == $xoopsModuleConfig['need_login']) {
    ...
    if ('1' == $xoopsModuleConfig['auto_id'] and !empty($xoopsUser)) {
...
function insert_tad_cbox()
{
    ...
    //姓名
    if ('1' == $xoopsModuleConfig['auto_id']) {
        if (empty($xoopsUser)) {
            $publisher = (empty($_POST['publisher']) or _MA_TADCBOX_DEFAULT_PUBLISHER == $_POST['publisher']) ? _MA_TADCBOX_DEFAULT_PUBLISHER : _MA_TADCBOX_DEFAULT_PUBLISHER . "_{$_POST['publisher']}";
        } else {
            $name = $xoopsUser->getVar('name');
            if (!empty($name)) {
                $publisher = $name;
    ...
    //留言
    if (empty($_POST['msg']) or _MA_TADCBOX_MSG == $_POST['msg']) {
        return;
    }
    $str_num = mb_strlen(strip_tags($_POST['msg']));
    ...
    $msg = ('1' == $xoopsModuleConfig['allow_html']) ? $myts->addSlashes($_POST['msg']) : $myts->htmlSpecialChars($myts->addSlashes($_POST['msg']));
